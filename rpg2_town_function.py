...
def inn(p_pc, ib_pc, h_p):
        ...
        if check.upper() == "R":
                ...
                choice = raw_input("Warrior, Mage, Cleric, or Summoner? C/M/S/W? ")
                if choice.upper() == "C":
...
def practice_arena(p_pc, ib_pc):
        ...
        if p_pc.level < C.LEVEL_LIMIT:
                ...
                check = raw_input("Want to train? Y/N ")
                ...
                if check.upper == "A" and ib_pc.coins >= C.LEVEL_PRICE * (p_pc.atkbonus ** C.INCREASE_EXPONENT):
                        ...
                        p_pc.defbonus += 1
                elif check.upper == "M" and ib_pc.coins >= C.LEVEL_PRICE * (p_pc.mana ** C.INCREASE_EXPONENT):
...
def potion_store(ib_pc):
        ...
        check = raw_input("Health, Mana, or Boost? H/M/B")
        if check.upper() == "H":
                try:
                        quantity = int(raw_input("How many do you want? "))
...
def equipment_store(p_pc, ib_pc):
        ...
        if check.upper() == "A":
                try:
                        quality = int(raw_input("What quality level? "))
...
def town(p_pc, ib_pc, h_p):
        ...
        check = raw_input("LEAVE, INN, PRACTICE ARENA, STORES? L/I/P/S")
        if check.upper() == "L":
                ...
                store = raw_input("EQUIPMENT, POTION? E/P ")
                if store.upper() == "E":
