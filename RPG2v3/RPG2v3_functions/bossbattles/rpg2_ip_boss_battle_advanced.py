...
def pick_random_hero(h_p):
        ...
        return hero
#egg actions
def egg_action(m_npc, x, b_p, h_p, ib_pc):
        print("The icy storm rages! ")
        y = (1 + B.I_P_L - x) ** 2
        m_npc.skill += 1
        for hero in h_p:
                hero.health -= y
                hero.atkbonus -= min(y, hero.atkbonus)
                hero.defbonus -= min(y, hero.defbonus)
                hero.skill -= min(y, hero.skill)
                ...
                if len(h_p) == 0:
                        ...
                        for mon in b_p:
                                if mon.name == "Cryo Phoenix" and mon.health > 0:
                                        ...
                                        egg_action(mon, lives, b_p, h_p, ib_pc)
                for mon in b_p:
